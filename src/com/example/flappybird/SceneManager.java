...
public class SceneManager {
	...
	public Scene createScene(){
		...
		// bird		
		float birdStartXOffset = (FlappyBirdActivity.CAMERA_WIDTH / 4) - (Bird.BIRD_WIDTH / 4);
		float birdYOffset = (FlappyBirdActivity.CAMERA_HEIGHT / 2) - (Bird.BIRD_HEIGHT / 4);
		...
		// you suck text		
		mYouSuckText = new Text(0, FlappyBirdActivity.CAMERA_HEIGHT / 2 - 100, mResourceManager.mYouSuckFont, "You Suck!", new TextOptions(HorizontalAlign.CENTER), mContext.getVertexBufferObjectManager());
	...
	public static void centerSprite(Sprite sprite){
		sprite.setX((FlappyBirdActivity.CAMERA_WIDTH / 2) - (sprite.getWidth() / 2));	
		sprite.setY((FlappyBirdActivity.CAMERA_HEIGHT / 2) - (sprite.getHeight() / 2));	
	}
	...
	private void centerText(Text text){
		text.setX((FlappyBirdActivity.CAMERA_WIDTH / 2) - (text.getWidth() / 2));		
	}
}
